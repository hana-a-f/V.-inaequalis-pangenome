...
5. QC — BUSCO on the protein sets
6. Effector-focused annotation with **miniprot** using Rocafort et al. AVR homologs
7. Extract CDS from both BRAKER and miniprot with **gffread**
8. QC the CDS with the custom `gff_qc.mod.py` script
9. Filter by QC flags (frameshifts, internal stops, missing start codons, short ORFs)
10. Merge BRAKER + miniprot annotations per isolate with **gffread -M**
11. Reduce merged annotations to longest isoform per gene (AGAT)
12. Collapse overlapping transcripts to one locus representative with `collapse_longest_isoform_by_locus.py`
13. Final naming — proteins and BEDs renamed to `ISOLATE.gene` format for GENESPACE later
14. Final BUSCO on the collapsed protein sets
15. Compute annotation stats — genes per isolate, per chromosome, per Mb
...
The following 7 paired-end RNA-Seq accessions were downloaded and trimmed prior to alignment, Rocafort et al 2023
...
## Step 8 — miniprot: align curated effector homologs 

Targeted annotation of effectors (from Rocafort et al. 2023 and Sannier et al 2025 (preprint)), which are prone to being missed.
